clear;
addpath ../RiemannianMC/Packages
addpath ../RiemannianMC/Packages/LMaFit
addpath ../RiemannianMC/Packages/LMaFit/lmafit
addpath ../RiemannianMC/Packages/LMaFit/Utilities
addpath ../RiemannianMC/Packages/Manifold
addpath ../RiemannianMC/Auxiliary
addpath ../RiemannianMC/Solvers
addpath ../RiemannianMC/Projection_Omega
...
n = 5000;
k = 5;
eps = 0.02; % fraction of observed entries
noise = 0.0;
...
case 1
  ...
  [U0 junk] = qr(rand(m,k),0); [V0 junk] = qr(rand(n,k),0);
...
figure;  plot(0:size(hist.inc,1)-1,hist.inc(:,1), 'b');
hold on; plot(0:size(hist.inc,1)-1,hist.inc(:,2), 'r');
hold on; plot(0:size(hist.inc,1)-1,hist.inc(:,3), 'g');
hold on; plot(0:size(hist.inc,1)-1,hist.inc(:,4), 'k');
figure;  plot(0:size(hist.dst,1)-1,hist.dst(:,1), 'b');
hold on; plot(0:size(hist.dst,1)-1,hist.dst(:,2), 'r');

% run other methods here
% ...
fprintf('\nRiemann-CG\n');
bt = tic;
[X stats] = riemanncg(A0,K,k,'maxit',maxit,'rel_f_tol',tol,'beta_type','P-R');
meth_name{end+1} = 'Riemann-CG'; meth_obj{end+1} = stats.rmse; meth_time{end+1} = stats.all_times;
toc(bt);
