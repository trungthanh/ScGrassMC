function [U S V hist] = scaled_grass_mc_adapt(K, A_K, r, varargin)
...
beta = 0;
total_time = 0;
if isfield(params, 'U0')
  subsp_dist = [subsp_dist; norm(params.U0*params.U0'-x.U*x.U','fro')];
end
for itc=1:maxit
  ...
  if isfield(params, 'U0')
    subsp_dist = [subsp_dist; norm(params.U0*params.U0'-xc_new.U*xc_new.U','fro')];
  end
...
[m n] = size(K);
k = size(x.U,2);
[z.U Ru] = qr([x.U+t*h.U x.U],0);
[z.V Rv] = qr([x.V+t*h.V x.V],0);
if is_dir
  return;
end
switch sigma_type
