...
%[x.U junk] = qr(rand(m,r), 0);
%[x.V junk] = qr(rand(n,r), 0);
...
inchist = zeros(maxit, 2);
dsthist = zeros(maxit, 4);
...
for itc=1:maxit
  ...
  [UU junk] = qr(xc.U,0); [VV junk] = qr(xc.V, 0);
  inchist(itc, :) = [incoherence(UU), incoherence(VV)];
  %dsthist(itc, :) = [subsp_dist(UU,get_params(params,'U0',0)), subsp_dist(VV,get_params(params,'V0',0)), subsp_dist_proj(UU,get_params(params,'U0',0)), subsp_dist_proj(VV,get_params(params,'V0',0))];
  %fprintf('inc(U) = %f %f, inc(V) = %f %f\n', inchist(itc,1), inchist(itc,3), inchist(itc,2), inchist(itc,4));
...
hist.inc = inchist(1:itc,:);
hist.dst = dsthist(1:itc,:);
...
%lambda = 2; % CHANGE THIS for fixec step_size
